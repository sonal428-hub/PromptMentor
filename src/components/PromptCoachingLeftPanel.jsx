...
import { Sparkles, CheckCircle2, XCircle, ArrowRight, Zap, RefreshCw, Layers, Edit3, HelpCircle } from 'lucide-react';
...
export default function PromptCoachingLeftPanel({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-slate-950/40 border-r border-white/5 relative">
      {/* ─────────────────────────────────────────────────────────────────
          TOP SCROLLABLE AREA: Analysis, Live Coach Advice & Concept Checklist
         ───────────────────────────────────────────────────────────────── */}
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-5">
        ...
        {/* Diagnostic Concept Checklist / Tags */}
        <div className="glass-panel p-4 space-y-3 border-violet-500/20">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400 font-mono">
              {coaxResult ? `${coaxResult.tags.filter(t => t.status === 'pass').length} / ${coaxResult.tags.length} Met` : 'Awaiting Coax'}
            </span>
          </div>
          ...
          <div className="text-xs text-gray-200 leading-relaxed font-sans min-h-[70px] flex items-center">
            {isCoaxing ? (
              ...
              <p className="text-gray-400 italic">
                Enter your draft prompt in the input bar below and click <strong>Coax ✓</strong> to receive live AI advice and targeted prompt improvements.
              </p>
            )}
          </div>

          {/* Action buttons: Use Suggested / Dismiss */}
          {coaxResult && (
      ...
      {/* ─────────────────────────────────────────────────────────────────
          PINNED BOTTOM PROMPT BAR: Input field + Coax ✓ Button
         ───────────────────────────────────────────────────────────────── */}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            <textarea
              ...
              className="flex-1 bg-slate-900/90 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-gray-100 placeholder-gray-500 focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/30 resize-none font-sans leading-relaxed transition-all"
            />
            <button
              onClick={onCoax}
              disabled={isCoaxing || !userPrompt.trim()}
              className="btn-primary text-xs py-3 px-5 rounded-xl flex items-center gap-2 font-bold disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap shadow-lg shadow-violet-600/30 self-stretch"
            >
              {isCoaxing ? (
                <>
